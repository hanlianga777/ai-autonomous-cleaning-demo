...
- DashScope Key 已在本机本地 `.env` 配置并完成一次真实 Scenario 02 主图复核：Qwen-VL 返回 `liquid` / 0.95 / `need_clean=true`，现有 Scheduler 仍选 Robot B；浏览器 Console 0 error。不得记录或提交 Key。该结果不代表 REAL YOLO、真实多视角或后验 REAL 验收已完成。
