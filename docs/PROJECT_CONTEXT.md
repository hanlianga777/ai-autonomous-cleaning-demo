...
## 34. 集成演示 V1 当前状态（2026-08-29）

客户入口统一为 `/` 与 `/prototype`。四个页面均可用：工作台展示两路监控、设备资产/二维白模、实时工单时间线；事件中心读取 SQLite；运营分析把运行事件叠加到 30 天基线；高级模式展示模型、映射和审计而不暴露 Key。

已实跑：Demo01 云端判定后由 Robot A 闭环；Demo02 首轮 0.61、独立复核 0.95、融合 0.97 后由 Robot B 闭环；Demo03 已进入 Robot C 与验收，但该次验收未满足闭环门控；Demo04 首轮 `need_clean=false` 正确转人工，人工完成后云端验收 0.98 并 CLOSED。真实模型输出仍有波动，LIVE 演示不得承诺固定结果。
