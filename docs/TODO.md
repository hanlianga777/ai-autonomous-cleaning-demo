...
- [ ] **真实串行执行**：把“后端一次性执行结果 + 前端 setTimeout 播放”替换为每阶段真实完成后才触发下一阶段的状态驱动方式；明确云端调用、定位、能力匹配、派单、路线、清洁、验收的时序。
- [ ] **真实 assignment projection**：工作台地图和机器人动作必须唯一由真实 `assignment_decision` 驱动，不能主要依赖 `scenario.robot`。
- [ ] **空间拓扑与路线**：建立可复用拓扑锚点（B1/B2 电梯、连廊入口/出口、A2 目标、室外道路、待机点），以 Phase 2 路由结果渲染路线，确保 Robot C 不穿墙/漂浮/飞线。
- [ ] **首轮 Demo02 Prompt**：首次多图云端 Prompt 明确同地点、同时间段、同地面区域、三个固定摄像头、不同视角，并要求判断反光/空间一致性/清洁能力；二次 Prompt 已有这部分但不能替代首轮。
- [ ] **重新进行真实 E2E 回归**：改动门控后的 Demo01–04、Demo04 人工验收、云端不可用/超时均需重新真实记录；不能把 2026-08-28 历史测试当作新逻辑验收。
- [ ] **文档一致性持续守护**：后续每次代码改动必须更新六份事实源，禁止旧“固定 Scenario 分离”“Qwen secondary”“四宫格 prototype”结论重新出现为有效要求。
