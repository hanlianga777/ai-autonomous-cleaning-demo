...
### Prototype V2｜视觉布局与交互纠偏（SUPERSEDED）

Prototype V1 **未通过人工视觉验收**，不得描述为最终产品设计已验收。主要问题：摄像头比例不合理、视频裁切、YOLO 证据展示不足、2.5D 建筑模型表达错误、验收证据重复、一级导航缺失，以及页面纵向空间浪费。

- [x] 恢复仅作视觉用途的左侧一级导航
- [x] 删除 Prototype 顶部无业务价值的 Hero 留白
- [x] 改为 16:9 监控墙和 4:3 源图的受控 letterbox 坐标层
- [x] 重做透明轴测园区建筑模型与 Robot C 跨楼路径位置
- [x] 删除 CLOSED 阶段对清洁前后图片的重复展示
- [x] 完成浏览器截图与交互验收：摄像头墙、多视角、Robot C 连廊、闭环去重、Console 0 error
- [ ] 等待新的人工视觉验收；未经确认不得整合进正式产品

### Integrated Demo V1｜真实 Qwen 语义链路（当前待人工验收）

- [x] `/prototype` 调用 `/api/demo-v1/runs/{demoId}`；LIVE 失败明确停在人工复核
- [x] 受控边缘 bbox / 固定置信度、Qwen 语义输出、TaskProfile、既有 Capability Engine / Scheduler 串联
- [x] Scenario 02 复用 Multi-view Agent，最多 2 台补充摄像头，并以一次三图 Qwen 请求研判
- [x] 在实际派发后调用 Qwen 清洁前/后验收；禁止仅因 after 图片存在而闭环
- [x] 按用户提供的机器人与园区白模素材构建固定双监控墙和空间叠加层
- [x] 提供用户显式可选的“手动稳定回放”，与 LIVE API 结果分离
- [x] 新增集成服务单元测试、FastAPI 端点检查、前端生产构建、浏览器初始布局/控制台检查
- [ ] 人工验收 LIVE 现场演示视觉、Qwen 门控策略与四组实际云端结果
- [ ] 当真实 Qwen 样本通过 `.85` 自动派发门控后，补做真实“派发 → Qwen 清洁后验收 → CLOSED”线上 E2E；当前所有真实样本均按规则停在人工复核
