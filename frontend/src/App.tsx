...
const navItems: { icon: typeof LayoutDashboard; label: string; view?: "dashboard" | "ai-lab" }[] = [
...
function App() {
  ...
  const [view, setView] = useState<"dashboard" | "ai-lab">("dashboard");
    ...
    <div className="min-h-screen bg-[#f6f7f8] text-slate-900">
      <aside className="fixed inset-y-0 left-0 z-20 hidden w-[238px] border-r border-slate-200 bg-white lg:flex lg:flex-col">
        ...
        <nav className="space-y-1 px-3 py-5">
          {navItems.map(({ icon: Icon, label, view: itemView }) => <button key={label} onClick={() => itemView && setView(itemView)} aria-disabled={!itemView} title={itemView ? undefined : "将在后续阶段实现"} className={`flex w-full items-center gap-3 rounded-sm px-3 py-2.5 text-left text-sm transition-colors ${view === itemView ? "bg-slate-900 font-medium text-white" : "text-slate-600 hover:bg-slate-100 hover:text-slate-900"} ${!itemView ? "cursor-default opacity-55" : ""}`}><Icon size={17} strokeWidth={1.7} />{label}</button>)}
        </nav>
      ...
      <main className="lg:ml-[238px]">
        <header className="flex h-[68px] items-center justify-between border-b border-slate-200 bg-white px-5 lg:px-8">
          <div className="flex items-center gap-3"><button className="lg:hidden"><Menu size={20} /></button><div><p className="section-kicker">{view === "ai-lab" ? "Independent perception test" : "Operations dashboard"}</p><h1 className="text-base font-semibold">{view === "ai-lab" ? "AI 感知验证" : "自主清洁运营总览"}</h1></div></div>
