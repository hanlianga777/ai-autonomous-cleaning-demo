...
import { AlertTriangle, Bell, ChevronDown, CircleHelp, LayoutDashboard, Menu, Settings2 } from "lucide-react";
...
type View = "workbench" | "dashboard" | "ai-lab" | "optimization" | "event-center" | "orchestration";
const navItems: { icon: typeof LayoutDashboard; label: string; view: View; technical?: boolean }[] = [
  { icon: LayoutDashboard, label: "任务工作台", view: "workbench" },
  { icon: Settings2, label: "技术后台", view: "dashboard", technical: true },
  { icon: CircleHelp, label: "AI 能力验证", view: "ai-lab", technical: true },
];
    ...
    <div className="min-h-screen bg-[#f6f7f8] text-slate-900">
      ...
      <main className="lg:ml-[238px]">
        <header className="flex h-[68px] items-center justify-between border-b border-slate-200 bg-white px-5 lg:px-8">
          <div className="flex items-center gap-3"><button className="lg:hidden"><Menu size={20} /></button><div><p className="section-kicker">{view === "workbench" ? "Customer demo workbench" : view === "ai-lab" ? "Independent perception test" : view === "optimization" ? "Analytics + Optimization" : view === "event-center" ? "Scenario launcher + Decision trace" : view === "orchestration" ? "Fleet + route explainability" : "Technical operations"}</p><h1 className="text-base font-semibold">{view === "workbench" ? "自主清洁任务工作台" : view === "ai-lab" ? "AI 能力验证" : view === "optimization" ? "运营优化中心" : view === "event-center" ? "AI 事件中心" : view === "orchestration" ? "机器人编排" : "技术后台 · 运营总览"}</h1></div></div>
        ...
        <div className="mx-auto max-w-[1600px] p-5 lg:p-8">
          ...
          {["dashboard", "event-center", "orchestration", "optimization"].includes(view) && <section className="mb-5 flex flex-wrap gap-2 border-b border-slate-200 pb-4"><p className="mr-2 w-full text-xs text-slate-500 sm:w-auto sm:pt-2">技术后台：</p>{([ ["dashboard", "运营总览"], ["event-center", "工作流与调度"], ["orchestration", "机器人编排"], ["optimization", "运营分析"] ] as const).map(([itemView, label]) => <button key={itemView} onClick={() => setView(itemView)} className={`rounded-sm border px-3 py-1.5 text-xs ${view === itemView ? "border-slate-800 bg-slate-800 text-white" : "border-slate-200 bg-white text-slate-600 hover:bg-slate-50"}`}>{label}</button>)}</section>}
