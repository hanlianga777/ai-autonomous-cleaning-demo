...
export function FloatingRobotOperationsAgent({ pageContext }: { pageContext: PageContext }) {
  ...
  return <section className="fixed z-[70] flex h-[min(560px,calc(100dvh-32px))] w-[min(420px,calc(100vw-32px))] flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl" style={{ left: position.x, top: position.y }} aria-label="园区运营助手完整对话窗口"><header className="flex shrink-0 items-center justify-between border-b border-slate-200 bg-slate-900 px-4 py-3 text-white"><div className="flex items-center gap-2.5"><span className="flex h-9 w-9 items-center justify-center rounded-full bg-sky-400 text-slate-900"><Bot size={19} /></span><div><p className="text-[14px] font-semibold">园区运营助手</p><p className="text-[11px] text-slate-300">协助查询事件与执行进度</p></div></div><button type="button" onClick={toggleExpanded} className="rounded-lg p-2 text-slate-300 hover:bg-white/10 hover:text-white" aria-label="收起为 AI 悬浮球"><X size={18} /></button></header><RobotOperationsChat pageContext={pageContext} /></section>;
}
export function AnalyticsAgentChat({ pageContext }: { pageContext: PageContext }) {
  return <aside className="sticky top-0 flex h-[calc(100dvh-54px)] min-h-[560px] flex-col border-l border-slate-200 bg-white" aria-label="园区运营助手固定对话"><header className="border-b border-slate-200 px-4 py-4"><div className="flex items-center gap-2"><span className="flex h-8 w-8 items-center justify-center rounded-full bg-slate-900 text-white"><Bot size={16} /></span><div><p className="text-[14px] font-semibold text-slate-900">园区运营助手</p><p className="text-[11px] text-slate-500">协助查询事件与执行进度</p></div></div></header><RobotOperationsChat pageContext={pageContext} /></aside>;
}
...
export function AnalyticsAdviceCards() {
  ...
  return <section className="mt-4" aria-label="AI 运营建议"><div className="mb-2"><p className="text-[12px] font-semibold text-slate-900">AI 运营建议</p><p className="mt-0.5 text-[12px] text-slate-500">基于近 30 天运营情况生成</p></div><div className="grid gap-3 lg:grid-cols-3">{items.length ? items.map((item, index) => <article key={`${item.finding}-${index}`} className="rounded-2xl border border-slate-200 bg-white p-4"><p className="text-[13px] font-semibold text-slate-900">运营建议 {index + 1}</p><p className="mt-2 text-[12px] leading-5 text-slate-700">{customerAgentMessage(item.recommendation)}</p></article>) : <article className="rounded-2xl border border-slate-200 bg-white p-4 text-[13px] text-slate-500">暂无可用的运营建议。</article>}</div></section>;
}
