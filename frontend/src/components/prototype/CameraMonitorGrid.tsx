...
export function CameraMonitorGrid({ event, onTrigger }: { event: ActiveEvent | null; onTrigger: (id: typeof scenarios[number]["id"]) => void }) {
  ...
  return <section className="flex min-h-0 flex-col border border-slate-200 bg-white" aria-label="固定摄像头监控">
    <div className="flex items-center justify-between border-b border-slate-100 px-3 py-2">
      <div><p className="text-sm font-semibold text-slate-900">固定摄像头监控</p><p className="mt-0.5 text-[10px] text-slate-500">4 路园区重点区域</p></div>
    ...
    <div className="grid min-h-0 grid-cols-2 gap-1 bg-slate-200 p-1">
      {cameraIds.map((id) => <CameraTile key={id} camera={cameras[id]} active={event?.scenario.cameraId === id && stage !== "CLOSED" && stage !== "IDLE"} supplemental={isMultiview && ["CAM-A1-02", "CAM-A1-04"].includes(id)} selected={selectedCamera === id} onSelect={() => setSelectedCamera(id)} />)}
    </div>
  </section>;
}
