...
export function CameraMonitorGrid({ event, onTrigger }: { event: ActiveEvent | null; onTrigger: (id: typeof scenarios[number]["id"]) => void }) {
  ...
  return <section className="surface-card flex min-h-0 flex-col overflow-hidden border border-slate-200 bg-white" aria-label="固定摄像头监控">
    <div className="flex shrink-0 items-center justify-between border-b border-slate-100 px-3 py-1.5">
      ...
      <div ref={menuRef} className="relative">
        ...
        {menuOpen && <div className="surface-inset absolute right-0 top-10 z-[70] w-72 border border-slate-200 bg-white p-2 shadow-lg">
    ...
    <div className="grid min-h-0 flex-1 grid-cols-3 gap-1 bg-slate-200 p-1">
      {monitorViews(event).map(({ camera, available, eventView, after }) => <div key={camera.id} data-camera-id={camera.id} data-evidence-role={after ? "after" : "before"} className="camera-rectangle relative min-h-0 overflow-hidden bg-slate-950">
