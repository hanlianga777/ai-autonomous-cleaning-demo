...
function FixedAgentReserve() {
  return <aside className="border-l border-slate-200 bg-white px-4 py-4" aria-label="Robot Operations Agent 预留区域">
    <p className="text-[10px] font-semibold tracking-[0.12em] text-slate-400">OPERATIONS ASSISTANCE</p>
    <section className="mt-3 border border-dashed border-slate-300 bg-slate-50 p-3.5"><p className="text-xs font-semibold text-slate-700">AI 运营优化建议</p><p className="mt-2 text-[11px] leading-5 text-slate-500">P1-F 尚未接入。此区域不会伪造建议、对话或机器人操作。</p></section>
    <section className="mt-3 border border-dashed border-slate-300 bg-slate-50 p-3.5"><p className="text-xs font-semibold text-slate-700">Robot Operations Agent</p><p className="mt-2 text-[11px] leading-5 text-slate-500">同一 Agent Session 将在 P1-F 接入；当前仅保留固定区域。</p></section>
  </aside>;
}

/** P1-E is a read-only projection of backend aggregates; no local KPI or recommendation is invented here. */
export function AnalyticsView() {
  ...
  return <main className="min-h-[calc(100vh-54px)] bg-[#f6f7f8] text-slate-800" aria-label="AI 自主清洁运营分析中心">
    <div className="grid min-h-[calc(100vh-54px)] grid-cols-1 xl:grid-cols-[minmax(0,1fr)_286px]"><section className="min-w-0 px-5 py-5 lg:px-7"><header className="flex flex-wrap items-end justify-between gap-3 border-b border-slate-200 pb-4"><div><p className="text-[10px] font-semibold tracking-[0.16em] text-slate-400">AI AUTONOMOUS CLEANING OPERATIONS ANALYSIS CENTER</p><h1 className="mt-1 text-xl font-semibold tracking-tight text-slate-900">AI 自主清洁运营分析中心</h1><p className="mt-1 text-xs text-slate-500">{filters.since || filters.until ? "自定义时间范围" : "近30天"} · 演示历史数据 + Runtime 增量，不代表客户生产运营数据。</p></div><button type="button" onClick={() => void load()} className="flex items-center gap-1.5 border border-slate-300 bg-white px-3 py-2 text-xs text-slate-700 hover:bg-slate-50"><RefreshCw size={14} />刷新统计</button></header>
      ...
      </> : <div className="min-h-[480px] border border-dashed border-slate-300 bg-white p-8 text-sm text-slate-500">尚未取得可验证的 Analytics API 响应。</div>}</section><FixedAgentReserve /></div>
  </main>;
}
