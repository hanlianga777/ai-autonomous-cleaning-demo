...
function RecentDemoInsight({ event, onViewArchive }: { event: ActiveEvent; onViewArchive?: (eventId: string) => void }) {
  const eventId = typeof event.liveResult?.event_id === "string" ? event.liveResult.event_id : "";
  const humanOutcome = event.liveResult?.assignment_decision && !(event.liveResult.assignment_decision as Record<string, unknown>).selected_robot_id;
  const outcome = humanOutcome ? "能力边界已识别并转入人工处置" : "已完成自动处置与验收";
  return <section aria-label="本次演示的运营价值" className="mt-4 border border-slate-200 bg-white px-4 py-3"><div className="flex items-start justify-between gap-3"><div><p className="text-[11px] font-medium text-slate-500">本次演示的运营价值</p><p className="mt-1 text-sm font-semibold text-slate-800">{event.scenario.demoCode} · {outcome}</p><p className="mt-1 text-xs leading-5 text-slate-600">{event.scenario.presentationFocus}。事件快照已保存至事件中心；本页 KPI 基于已返回的可追溯历史与运行增量，不将单次演示伪装成实时经营结论。</p></div>{eventId && onViewArchive && <button type="button" onClick={() => onViewArchive(eventId)} className="shrink-0 border border-slate-300 bg-white px-2 py-1 text-[12px] font-medium text-slate-700 hover:bg-slate-50">查看档案</button>}</div></section>;
}

export function AnalyticsView({ event, onViewArchive }: { event?: ActiveEvent | null; onViewArchive?: (eventId: string) => void }) {
