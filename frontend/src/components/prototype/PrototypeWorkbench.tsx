...
export function PrototypeWorkbench() {
  ...
  useEffect(() => {
    ...
    const next = state === "DETECTED" ? ["edge-review", "EDGE_DETECTED"] as const
      : state === "EDGE_DETECTED" ? [event.scenario.id === "liquid" ? "multi-view" : "cloud-review", event.scenario.id === "liquid" ? "MULTI_VIEW" : "CLOUD_REVIEW"] as const
      : state === "MULTI_VIEW" ? ["cloud-review", "CLOUD_REVIEW"] as const
