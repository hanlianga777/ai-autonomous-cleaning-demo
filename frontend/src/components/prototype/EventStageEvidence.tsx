...
export function EventStageEvidence({ event, entry, mode, onCompleteManual }: { event: ActiveEvent; entry: TimelineEntry; mode: "live" | "history"; onCompleteManual?: () => void }) {
  ...
  switch (entry.state) {
    ...
    case "LOCATED": {
      ...
      return <div className={card}><p><strong>事件位置：</strong>{location.building}栋{location.floor} · {customerTerm(location.zone)}</p><p><strong>SLAM坐标：</strong>X {location.x} / Y {location.y}</p><p><strong>所属地图：</strong>{routeLabel(String(location.map_id ?? ""))}</p></div>;
    }
    ...
    case "CLOSED": return <div className="mt-2 border border-emerald-200 bg-emerald-50 p-2 text-[12px] leading-5 text-emerald-800"><p className="font-semibold">AI 验收通过，事件已闭环</p><p>执行方：{result.assignment_decision?.selected_robot_name ?? (entry.detail.manual_completion ? "人工处置" : "历史记录未注明")}</p><TerminalFleet result={result} /></div>;
