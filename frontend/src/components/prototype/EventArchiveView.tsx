...
import {
  ...
  archiveDateTimeInputValue, archiveTimestampMs, archiveUrlWithSelection, canRenderArchiveDetail, durationLabel, eventTypeLabel, nextKnownEventBatch, parseArchiveFilters, parseArchiveSelection, structuralLocationLabel,
...
function EventRow({ item, selected, onSelect }: { item: ArchiveItem; selected: boolean; onSelect: (eventId: string) => void }) {
  return <button type="button" onClick={() => onSelect(item.event_id)} className={`w-full border-b border-slate-100 px-4 py-3 text-left transition-colors ${selected ? "bg-slate-100/90 shadow-[inset_2px_0_0_#334155]" : "bg-white hover:bg-slate-50"}`}>
    ...
    <div className="mt-2 flex min-w-0 items-center gap-2 overflow-hidden text-[10px] text-slate-500"><span className="truncate">发现 {formatTime(item.discovered_at)}</span><span className="h-3 w-px shrink-0 bg-slate-200" /><span className="truncate">{item.executor || "系统处理中"} · {durationLabel(item.duration_seconds)}</span></div>
  </button>;
}
...
export function EventArchiveView({ onAgentContextChange }: { onAgentContextChange?: (context: Record<string, unknown>) => void }) {
  ...
  useEffect(() => {
    const onPopState = () => { setSelectedId(parseArchiveSelection(urlHere())); setFilters(parseArchiveFilters(urlHere())); };
  ...
  return <main className="min-h-full bg-[#f6f7f8] px-5 py-5 text-slate-800" aria-label="事件中心">
    <div className="mx-auto max-w-[1540px]"><header className="mb-4"><h1 className="text-xl font-semibold tracking-tight text-slate-900">事件中心</h1><p className="mt-1 text-xs text-slate-500">按发现时间查看已保存的处置进度与验收结果</p></header>
      <section className="border border-slate-200 bg-white"><div className="flex flex-wrap items-center gap-2 border-b border-slate-100 px-3 py-2"><label className="flex min-w-[230px] flex-1 items-center gap-2 border border-slate-200 bg-slate-50 px-2.5 py-1.5 text-slate-500"><Search size={14} /><input aria-label="搜索事件" value={filters.q} onChange={(event) => updateFilters({ q: event.target.value })} placeholder="搜索事件类型或发生位置" className="min-w-0 flex-1 bg-transparent text-xs text-slate-700 outline-none placeholder:text-slate-400" /></label><label className="text-[11px] text-slate-500">事件类型 <select aria-label="事件类型" value={filters.eventType} onChange={(event) => updateFilters({ eventType: event.target.value })} className="ml-1 border border-slate-200 bg-white px-2 py-1.5 text-[11px] text-slate-700 outline-none">{EVENT_TYPES.map((type) => <option key={type || "all"} value={type}>{type ? eventTypeLabel(type) : "全部"}</option>)}</select></label><label className="text-[11px] text-slate-500">处置方式 <select aria-label="处置方式" value={filters.handlingMode} onChange={(event) => updateFilters({ handlingMode: event.target.value as HandlingMode })} className="ml-1 border border-slate-200 bg-white px-2 py-1.5 text-[11px] text-slate-700 outline-none">{HANDLING_MODES.map((mode) => <option key={mode.id || "all"} value={mode.id}>{mode.label}</option>)}</select></label></div>
        ...
        <div className="grid h-[calc(100vh-205px)] min-h-[460px] grid-cols-[minmax(0,1fr)_44%] overflow-hidden"><section className="flex min-h-0 min-w-0 flex-col border-r border-slate-200"><div className="grid grid-cols-3 border-b border-slate-100 px-4 py-2 text-[10px] font-medium text-slate-400"><span>事件 / 发生位置</span><span>发现时间 / 处置方式</span><span>执行对象 / 状态</span></div><div className="min-h-0 flex-1 overflow-y-auto">{items.length ? items.map((item) => <EventRow key={item.event_id} item={item} selected={item.event_id === selectedId} onSelect={selectEvent} />) : <div className="flex min-h-[400px] flex-col items-center justify-center px-8 text-center"><History size={20} className="text-slate-300" /><p className="mt-3 text-sm text-slate-600">当前筛选下没有事件</p></div>}</div><div className="flex items-center justify-between border-t border-slate-100 px-4 py-3 text-[11px] text-slate-500"><span>{total ? `${filters.offset + 1}–${offsetEnd} / ${total}` : "0 / 0"}</span><div className="flex gap-1"><button type="button" aria-label="上一页事件" disabled={filters.offset === 0} onClick={() => updateFilters({ offset: Math.max(0, filters.offset - filters.limit) })} className="border border-slate-200 p-1 disabled:cursor-not-allowed disabled:opacity-40"><ChevronLeft size={14} /></button><button type="button" aria-label="下一页事件" disabled={offsetEnd >= total} onClick={() => updateFilters({ offset: filters.offset + filters.limit })} className="border border-slate-200 p-1 disabled:cursor-not-allowed disabled:opacity-40"><ChevronRight size={14} /></button></div></div></section>
